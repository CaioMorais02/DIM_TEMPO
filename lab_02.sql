...
BEGIN
	...
	WHILE (@@FETCH_STATUS = 0)
	BEGIN
		DECLARE @MATRICULA_DIM INT, @NOME_DIM VARCHAR(60), @CPF_DIM VARCHAR(11), @CARGO_DIM VARCHAR(50), @FUNCAO_DIM VARCHAR(50), @SALARIO_DIM NUMERIC(10, 2), @SETOR_DIM VARCHAR(50), @DEPARTAMENTO_DIM VARCHAR(50)

		DECLARE C_DIM CURSOR FOR
		SELECT MATRICULA, NOME, CPF, CARGO, FUNCAO, SALARIO, SETOR, DEPARTAMENTO FROM DIM_FUNCIONARIO
		OPEN C_DIM
		FETCH C_DIM INTO @MATRICULA_DIM, @NOME_DIM, @CPF_DIM, @CARGO_DIM, @FUNCAO_DIM, @SALARIO_DIM, @SETOR_DIM, @DEPARTAMENTO_DIM

		IF @MATRICULA_DIM IS NULL
		BEGIN
		...
		BEGIN
			WHILE (@@FETCH_STATUS = 0)
			BEGIN
				IF @DT_CARGA = @DATA_CARGA
				BEGIN
					IF @MATRICULA != @MATRICULA_DIM
					BEGIN
						INSERT INTO DIM_FUNCIONARIO VALUES (@MATRICULA, @NOME, @CPF, @CARGO, @FUNCAO, @SALARIO, @CODIGO_SETOR, @SETOR, @CODIGO_DEPARTAMENTO, @DEPARTAMENTO, @DATA_CARGA, NULL, 'SIM')
					END
					...
					BEGIN
						...
						IF @DEPARTAMENTO != @DEPARTAMENTO_DIM
						BEGIN
							UPDATE DIM_FUNCIONARIO
							SET DT_FIM = @DATA_CARGA, FL_CORRENTE = 'NAO'
							WHERE MATRICULA = @MATRICULA
	...
	CLOSE C_STAGING
	DEALLOCATE C_STAGING

END

EXEC SP_DIM_FUNCIONARIO '2024-02-18';
